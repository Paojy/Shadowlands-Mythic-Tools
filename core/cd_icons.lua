...
local function CreateIcon(f)
	...
	icon.cd:SetScript("OnShow", function()
		if not party_cd['Roster'][icon.player_name][icon.spellID] then return end
			...
			for i=1, 8 do
				for j=1, 5 do
					...
					if uf and uf.unit and UnitIsUnit(uf.unit, unit) then
						if SMT_CDB["CD_Icons"]["grow_dir"] == "LEFT" then
...
T.EditCDBar = function(tag)
	for i = 1, #party_cd.Frames do
		...
		if tag == "show" then
			if not SMT_CDB["General"]["disable_all"] and SMT_CDB["CD_Icons"]["enable"] then
				f:Show()
			else
				f:Hide()
			end
